...
function setup() {{
  var canvas = createCanvas(windowWidth * 0.95, 400);
...
function draw() {{
  ...
  for (let x = 0; x <= width; x += 5) {{
    ...
    let distToBank = max(0, x - width * 0.4);
    let localAmp = (hs * 40) * (1 + (distToBank * 0.003));
    
    let xOffset = 0;
    if (sin(cycle) > 0.4 && xi > 1.1) {{
        xOffset = pow(sin(cycle), 3) * (xi * 20); // The "Ledge" overhang
    }}

    let y = (height - 120) - (tide * 20) + sin(cycle) * localAmp;
    let sandY = height - (20 + (1 / (1 + exp(-0.02 * (x - width*0.7)))) * (slope * 2500));
    y = min(y, sandY); // Clamp water to sand
    vertex(x + xOffset, y);
  }}
  vertex(width, height);
  endShape(CLOSE);
